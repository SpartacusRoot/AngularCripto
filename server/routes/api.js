...
router.get("/autocomplete", (req, res) => {
  ...
  const autocompleteTest =
    "SELECT clienti.nome_cliente, usernames.username,tipo_accesso.tipo_accesso FROM clienti JOIN usernames ON clienti.id = usernames.id JOIN tipo_accesso ON usernames.id_username = tipo_accesso.id_username JOIN password ON tipo_accesso.id_tipo_accesso = password.id_tipo_accesso";
...
router.get("/autocompleteAccesso", (req, res) => {
  // test autocomplete with new database refer autocompleteTest
  const autocompleteTest =
    " SELECT DISTINCT clienti.nome_cliente,tipo_accesso.tipo_accesso FROM clienti JOIN usernames ON clienti.id = usernames.id JOIN tipo_accesso ON usernames.id_username = tipo_accesso.id_username JOIN password ON tipo_accesso.id_tipo_accesso = password.id_tipo_accesso WHERE nome_cliente=($1)";
...
router.get("/check", (req, resp) => {
  ...
  const checkValue = [
    req.query.nome_cliente,
    req.query.username,
    req.query.tipo_accesso
  ];
  var client = new pg.Client(connectionString);
  client.connect(function(err) {
    if (err) {
    ...
    client.query(checkName, checkValue, function(err, res) {
      ...
      if (res.rows.length > 0) {
        res.rows.some(res => {
          if (
            res.nome_cliente === req.query.nome_cliente &&
            res.username === req.query.username &&
            res.tipo_accesso === req.query.tipo_accesso
          ) {
            return resp.send({
              error:
                "la combinazione nome cliente , username e tipo accesso è già esistente",
              status: true
            });
          } else {
...
router.post("/form", async (req, res) => {
  ...
  (async () => {
    ...
    try {
      ...
      userModel.name = await client.query(
        "INSERT INTO clienti(nome_cliente) VALUES($1) RETURNING id",
        [req.body.name]
      );
      userModel.username = await client.query(
        "INSERT INTO usernames(username,id) VALUES ($1,$2) RETURNING id_username;",
        [req.body.username, userModel.name.rows[0].id]
      );
      userModel.access = await client.query(
        "INSERT INTO tipo_accesso(tipo_accesso,id_username) VALUES ($1,$2) RETURNING id_tipo_accesso;",
        [req.body.access, userModel.username.rows[0].id_username]
      );
      userModel.pass = await client.query(
        "INSERT INTO password(password, note,id_tipo_accesso) VALUES ($1, $2, $3) RETURNING password;",
        [
          userModel.password,
          userModel.note,
          userModel.access.rows[0].id_tipo_accesso
        ]
      );
...
router.put("/update", (req, res) => {
  ...
  console.log(
    ...
    "note " + userModel.note,
    "id" + userModel.id,
    "accesso " + userModel.tipo_accesso
  );
  ...
  (async () => {
    ...
    try {
      if (req.body.password.length <= 16) {
        await client.query("BEGIN");
        userModel.nome_cliente = await client.query(
          "UPDATE clienti SET nome_cliente=($1) WHERE id=($2)",
          [userModel.nome_cliente, req.body.id]
        );
        userModel.username = await client.query(
          "UPDATE usernames SET username=($1) WHERE id_username=($2)",
          [req.query.username, userModel.id_username]
        );
        userModel.tipo_accesso = await client.query(
          "UPDATE tipo_accesso SET tipo_accesso=($1) WHERE id_tipo_accesso=($2)",
          [userModel.tipo_accesso, userModel.id_tipo_accesso]
        );
        userModel.password = await client.query(
          "UPDATE password SET password=($1),note=($2) WHERE id_password=($3)",
          [userModel.password, userModel.note, userModel.id_password]
        );
        console.log(
          "id " + userModel.id,
          "id_tipo_accesso " + userModel.id_tipo_accesso,
          "username " + req.query.username,
          "tipo_accesso " + userModel.tipo_accesso,
          "note " + userModel.note,
          "pass " + userModel.password,
          "id_password " + userModel.id_password
        );
        await client.query("COMMIT");
      } else {
        await client.query("BEGIN");
...
router.get("/search/", (req, res, next) => {
  var searchRes = {};
  let searchResult =
    "SELECT * FROM clienti JOIN usernames ON clienti.id = usernames.id JOIN tipo_accesso ON usernames.id_username = tipo_accesso.id_username JOIN password ON tipo_accesso.id_tipo_accesso = password.id_tipo_accesso WHERE TRUE=TRUE";
  let queryParams = [];

  req.query.nome_cliente && req.query.nome_cliente != ""
    ? ((searchResult += " AND nome_cliente = $1"),
      queryParams.push(req.query.nome_cliente))
    : null;
  req.query.tipo_accesso && req.query.tipo_accesso != ""
  ...
  req.query.password && req.query.password != ""
    ? ((searchResult += " AND password = $" + (queryParams.length + 1)),
      queryParams.push(req.query.password))
    : null;
